...
def save_checkpoint(model, optimizer, scheduler, epoch, path='checkpoint.pth', delete_symbol=False):
    ...
    print(f"Checkpoint saved at epoch {path}")
...
class Trainer:
    def __init__(self,
        ...
        self.num_step_per_epoch = 200 // self.num_gpus
        self.val_number = 20 // self.num_gpus
    ...
    def validate(self):
        ...
        if self.global_step % self.val_every == 0 \
            ...
            if self.local_rank == 0:
                ...
                self.validation_end(val_outputs_merge, self.model_save_path)
    ...
    def train(self,
        ...
        if self.is_adult:
            self.num_step_per_epoch = len(self.train_loader) * 2
            self.val_number = len(self.val_loader) * 2
        ...
        if self.resume_training == 'all':
            start_epoch = load_checkpoint(self.model, self.final_save_path)
        elif  self.resume_training == 'seglayer':
            self.model.load_state_dict(torch.load(self.final_save_path)['model_state_dict'], strict=False)
            
            for param in self.model.parameters():
                param.requires_grad = False
            for param in self.model.decoder.seg_layers.parameters():
                param.requires_grad = True
            print('resume from seglayer')
        elif self.resume_training == 'decoder':
            self.model.load_state_dict(torch.load(self.final_save_path)['model_state_dict'], strict=False)
            for param in self.model.parameters():
                param.requires_grad = False
            for param in self.model.decoder.parameters():
                param.requires_grad = True
            print('resume from decoder')
        elif self.resume_training == 'encoder':
            self.model.load_state_dict(torch.load(self.final_save_path)['model_state_dict'], strict=False)
            for param in self.model.parameters():
                param.requires_grad = False
            for param in self.model.encoder.parameters():
                param.requires_grad = True
            print('resume from encoder')
        elif self.resume_training == 'none':
            start_epoch = 0
        else:
            start_epoch = 0
    ...
    def train_epoch(self, 
        ...
        with tqdm(total=self.num_step_per_epoch, disable=(self.local_rank != 0)) as t:
            ...
            for i in range(self.num_step_per_epoch):
                ...
                t.update(1)
            avg_loss = total_loss / self.num_step_per_epoch
            self.log("epoch_avg_loss", avg_loss, epoch)  
