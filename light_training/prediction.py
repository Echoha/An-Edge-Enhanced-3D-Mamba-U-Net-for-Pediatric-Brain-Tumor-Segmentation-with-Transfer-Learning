...
class Predictor:
    ...
    def predict_noncrop_probability(model_output, properties):
        ...
        if len(model_output.shape) == 3:
            ...
            exit(0)
    def maybe_mirror_and_predict_ds(self, x, model, device=torch.device("cpu"), **kwargs) -> torch.Tensor:
        # mirror_axes = [0, 1, 2]
        ...
        with torch.no_grad():
            ...
            with torch.autocast("cuda", enabled=True) if device.type == "cuda" else dummy_context():
                prediction = window_infer(x, model, **kwargs)[0].cpu()
                ...
                if mirror_axes is not None:
                    ...
                    if 0 in mirror_axes:
                        prediction += torch.flip(window_infer(torch.flip(x, (2,)), model, **kwargs)[0], (2,)).cpu()
                        torch.cuda.empty_cache()
                    if 1 in mirror_axes:
                        prediction += torch.flip(window_infer(torch.flip(x, (3,)), model, **kwargs)[0], (3,)).cpu()
                        torch.cuda.empty_cache()
                    if 2 in mirror_axes:
                        prediction += torch.flip(window_infer(torch.flip(x, (4,)), model, **kwargs)[0], (4,)).cpu()
                        torch.cuda.empty_cache()
                    if 0 in mirror_axes and 1 in mirror_axes:
                        prediction += torch.flip(window_infer(torch.flip(x, (2, 3)), model, **kwargs)[0], (2, 3)).cpu()
                        torch.cuda.empty_cache()
                    if 0 in mirror_axes and 2 in mirror_axes:
                        prediction += torch.flip(window_infer(torch.flip(x, (2, 4)), model, **kwargs)[0], (2, 4)).cpu()
                        torch.cuda.empty_cache()
                    if 1 in mirror_axes and 2 in mirror_axes:
                        prediction += torch.flip(window_infer(torch.flip(x, (3, 4)), model, **kwargs)[0], (3, 4)).cpu()
                        torch.cuda.empty_cache()
                    if 0 in mirror_axes and 1 in mirror_axes and 2 in mirror_axes:
                        prediction += torch.flip(window_infer(torch.flip(x, (2, 3, 4)), model, **kwargs)[0], (2, 3, 4)).cpu()
                        torch.cuda.empty_cache()
    ...
    def save_to_nii(self, return_output,
        ...
        if return_output.ndim == 4:  # 确保数据是四维 (num_classes, depth, height, width)
            # 去掉类别维度并调整顺序
            return_output = np.squeeze(return_output)  # 变为 (155, 240, 240)
            # return_output = np.transpose(return_output, (2, 1, 0))  # 转换为 (240, 240, 155)
    ...
    def postprocess(self, model_output, et_threshold=0.9, min_size=10, min_voxels=20, wt_threshold_et=0.005, wt_threshold_ed=1.0):
        """
        后处理：去除小区域，并根据 ET/WT 和 ED/WT 比例重新定义标签
        :param model_output: 模型输出，标签图，形状为 (H, W, D)
        :param et_threshold: 增强肿瘤区域的概率阈值，默认 0.9
        :param min_size: 最小区域大小，小于该区域体积的区域将被重新定义为坏死区域，默认 16
        :param min_voxels: 增强肿瘤体素数量阈值，少于该数量的增强肿瘤体素将被重新定义为坏死区域，默认 73
        :param wt_threshold_et: ET/WT 阈值，默认 0.04
        :param wt_threshold_ed: ED/WT 阈值，默认 1.0
        :return: 后处理后的模型输出
        """
        # 1. 连通组件分析：去除小于 min_size 个体素的增强肿瘤区域
        labeled_output, num_labels = ndi.label(model_output)
        sizes = np.bincount(labeled_output.ravel())
        sizes[0] = 0  # 忽略背景区域

        for label in range(1, num_labels + 1):
            # 如果区域小于 min_size，且平均概率低于 et_threshold，替换为坏死区域
            region = model_output[labeled_output == label]
            # 使用 PyTorch 的 mean() 计算平均值
            if isinstance(region, torch.Tensor):  # 如果是 PyTorch 张量
                region = region.float()  # 转换为浮动类型
                region_mean = torch.mean(region).item()  # 使用 PyTorch 的 mean() 函数
            else:  # 如果是 NumPy 数组
                region_mean = np.mean(region)  # 使用 NumPy 的 mean() 函数
            
            if sizes[label] < min_size and region_mean < et_threshold:
                model_output[labeled_output == label] = 2  # 将区域标记为坏死区域（NCR）

        # 2. 根据增强肿瘤的体素数量判断：如果体素数目少于 min_voxels 且平均概率低于 et_threshold，则重新定义为坏死区域
        et_voxels = model_output == 3  # 假设 ET 标签是 3
        if torch.sum(et_voxels) < min_voxels and torch.mean(model_output[et_voxels].float()) < et_threshold:
            model_output[et_voxels] = 2  # 将 ET 替换为坏死区域（NCR）

        # 3. ET/WT 比例重新定义
        # ET: 标签为 3, WT: 标签为 1, 2, 3
        et_wt_ratio = torch.sum(model_output == 3) / (torch.sum(torch.isin(model_output, torch.tensor([1, 2, 3]))) + 1e-6)  # 计算 ET/WT 比例
        if et_wt_ratio < wt_threshold_et:
            model_output[model_output == 3] = 2  # 将 ET 标签重新定义为坏死区域（NCR）

        # # 4. ED/WT 比例重新定义
        # # ED: 标签为 1, WT: 标签为 1, 2, 3
        # ed_wt_ratio = torch.sum(model_output == 1) / (torch.sum(torch.isin(model_output, torch.tensor([1, 2, 3]))) + 1e-6)  # 计算 ED/WT 比例
        # if ed_wt_ratio < wt_threshold_ed:
        #     model_output[model_output == 1] = 2  # 将 ED 标签重新定义为坏死区域（NCR）
