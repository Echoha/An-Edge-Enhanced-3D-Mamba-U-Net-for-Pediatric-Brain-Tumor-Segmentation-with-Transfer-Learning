...
def get_train_transforms_ds(patch_size, mirror_axes=None, deep_supervision_scales=None):
    tr_transforms = []
    patch_size_spatial = patch_size
    ignore_axes = None
    angle = (-30. / 360 * 2. * np.pi, 30. / 360 * 2. * np.pi)
...
def get_validation_transforms_ds(deep_supervision_scales) -> AbstractTransform:
        val_transforms = []
        val_transforms.append(RemoveLabelTransform(-1, 0))

        # val_transforms.append(RenameTransform('seg', 'target', True))
        if deep_supervision_scales is not None:
            val_transforms.append(DownsampleSegForDSTransform2(deep_supervision_scales, 0, input_key='seg',
                                                               output_key='seg'))
        val_transforms.append(NumpyToTensor(['data', 'seg'], 'float'))
        val_transforms = Compose(val_transforms)
        return val_transforms
