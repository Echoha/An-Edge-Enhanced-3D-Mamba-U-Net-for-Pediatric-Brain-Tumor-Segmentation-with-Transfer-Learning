...
class MedicalDataset(Dataset):
    ...
    def __getitem__(self, i):
        ...
        properties['name'] = os.path.splitext(os.path.basename(self.datalist[i]))[0]
...
def my_train_val_test_loader_from_split_json(data_dir, splits_json_file):
    import json
    ...
    val_datalist = datalist["val"]
    test_datalist = datalist["test"]
    print(test_datalist)

    def add_pre_extension(datalist):
        for i in range(len(datalist)):
            datalist[i] = os.path.join(data_dir, datalist[i])
            datalist[i] = datalist[i] + '.npz'

    add_pre_extension(train_datalist)
    add_pre_extension(val_datalist)
    add_pre_extension(test_datalist)
...
def get_train_val_test_loader_from_train(data_dir, train_rate=0.7, val_rate=0.1, test_rate=0.2, seed=42):
    ...
    train_ids = [os.path.splitext(os.path.basename(path))[0] for path in train_datalist]
    val_ids = [os.path.splitext(os.path.basename(path))[0] for path in val_datalist]
    test_ids = [os.path.splitext(os.path.basename(path))[0] for path in test_datalist]

    data_list = {'train': train_ids, 'val': val_ids, 'test': test_ids}
    with open('/root/autodl-tmp/SegMamba-umamba/splits_ped.json', 'r+') as json_file:
