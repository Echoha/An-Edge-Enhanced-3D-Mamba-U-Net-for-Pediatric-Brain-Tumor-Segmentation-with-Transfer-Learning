...
class ResidualEncoder(nn.Module):
    def __init__(self,
        ...
        feature_map_sizes = []
        feature_map_size = input_size # [128, 128, 128], [64, 64, 64], [32, 32, 32], [16, 16, 16], [8, 8, 8], [4, 4, 4]
        # features_per_stage: 32, 64, 128, 256, 320, 320
        # t1: [1, 4, features_per_stages, z, w, h]
        for s in range(n_stages):
            feature_map_sizes.append([i // j for i, j in zip(feature_map_size, strides[s])])
            feature_map_size = feature_map_sizes[-1]
            # w * h * z < features per stage
...
class UNetResDecoder(nn.Module):
    ...
    def forward(self, skips):
        ...
        seg_outputs = seg_outputs[::-1] # 创建一个新的列表,其中包含seg_outputs中元素的逆序排列。
    ...
    def compute_conv_feature_map_size(self, input_size):
        ...
        for s in range(len(self.stages)):
            ...
            if self.deep_supervision or (s == (len(self.stages) - 1)):
                output += np.prod([self.num_classes, *skip_sizes[-(s+1)]], dtype=np.int64) # 这行代码的作用是将 self.num_classes 乘上 skip_sizes 列表中倒数第 (s+1) 个元素的乘积,并将结果累加到 output 变量上。
        return output
...
def get_enc_3d(
    ...
    segmentation_network_class_name = 'UNetEnc'
    network_class = UNetEnc
    kwargs = {
        'UNetEnc': {
